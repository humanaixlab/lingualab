...
export function buildReportExportModel({ context, analysis, language = "en" }) {
  ...
  if (context) {
    ...
    const entries = (context.analysisType === "frequency" || context.analysisType === "corpus-research")
    ...
    if ((context.analysisType === "concordance" || context.analysisType === "corpus-research") && Array.isArray(payload.contexts) && payload.contexts.length) {
      tables.push({
    ...
    if (context.analysisType === "corpus-research" && Array.isArray(payload.ngrams) && payload.ngrams.length) {
      const rows = cleanEntries(payload.ngrams);
      tables.push({ name: locale === "ar" ? "المتتاليات اللفظية" : "N-grams", headers: [locale === "ar" ? "المتتالية" : "Sequence", locale === "ar" ? "التكرار" : "Frequency"], rows });
    }
