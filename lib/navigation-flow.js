export const BUILD_HOME = "/research-planner#build-tools";
export const RESEARCH_HOME = "/research-planner";
export const ANALYZE_HOME = "/tools/analyze";
export const LEARN_HOME = "/learning-center";
...
export function reportReturnTarget(sourceTool, language = "en", pathId = null) {
  if (pathId === "corpus-linguistics") {
    return {
      href: "/research-paths/corpus-linguistics",
      label: language === "ar" ? "العودة إلى مسار لسانيات المدونات" : "Back to Corpus Linguistics path",
    };
  }
  const pathHomes = { "morphology-syntax": "/research-planner#morphology-syntax", semantics: "/research-planner#semantics", "discourse-pragmatics": "/research-planner#discourse-pragmatics", "text-classification": "/research-planner#build-tools", "information-extraction": "/research-planner#build-tools", "nlp-experiments": "/research-planner#build-tools" };
  if (pathHomes[pathId]) return { href: pathHomes[pathId], label: language === "ar" ? "العودة إلى المسار الحالي" : "Back to current workflow" };
