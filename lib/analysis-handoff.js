...
const SOURCES = new Set(["frequency", "concordance", "ngrams", "pos", "corpus-research"]);
const TYPES = new Set(["frequency", "concordance", "ngrams", "pos", "corpus-research"]);
const CORPUS_SOURCES = new Set(["frequency", "concordance", "ngrams", "corpus-research"]);
...
function cleanEvidence(type, value) {
  ...
  if (type === "corpus-research") return {
    ...
    query: cleanText(value.query, 500),
    frequencies: cleanEntries(value.frequencies),
    contexts: cleanStrings(value.contexts),
    size: value.size === 3 ? 3 : 2,
    ngrams: cleanEntries(value.ngrams),
  };
  return null;
}
...
export function createAnalysisHandoff(sourceTool, analysisType, input, storage = globalThis?.sessionStorage, now = Date.now()) {
  ...
  const handoff = {
    ...
    pathId: CORPUS_SOURCES.has(sourceTool) ? "corpus-linguistics" : null,
    sourceTool,
    analysisType,
    createdAt: now,
    text,
    evidence,
    returnHref: `${SOURCE_ROUTES[sourceTool]}?researchPath=corpus-linguistics&from=research-path&returnSection=corpus-linguistics&resultHandoff=${encodeURIComponent(id)}`,
  };
...
export function readAnalysisHandoff(search, storage = globalThis?.sessionStorage, now = Date.now()) {
  ...
  try {
    ...
    const supportedVersion = value?.version === 1 || value?.version === 2 || value?.version === 3;
    const valid = supportedVersion && value.id === id && value.destination === "interpreter" &&
    ...
    return text.trim() && evidence ? {
      id: value.id,
      pathId: CORPUS_SOURCES.has(value.sourceTool) ? "corpus-linguistics" : null,
      sourceTool: value.sourceTool,
      analysisType: value.analysisType,
      text,
      evidence,
      returnHref: value.returnHref || SOURCE_ROUTES[value.sourceTool],
    } : null;
  } catch {
    return null;
  }
...
export function interpretationContextText(handoff) {
  if (!handoff) return "";
  return JSON.stringify({ pathId: handoff.pathId, sourceTool: handoff.sourceTool, analysisType: handoff.analysisType, results: handoff.evidence });
}

export function readAnalysisResultHandoff(search, sourceTool, storage = globalThis?.sessionStorage, now = Date.now()) {
  const id = new URLSearchParams(search || "").get("resultHandoff");
  if (!id || !SOURCES.has(sourceTool) || !storage) return null;
  try {
    const raw = storage.getItem(ANALYSIS_HANDOFF_KEY);
    if (!raw || raw.length > 100000) return null;
    const value = JSON.parse(raw);
    const age = now - value?.createdAt;
    const valid = value?.version === 3 && value.id === id && value.sourceTool === sourceTool &&
      value.destination === "interpreter" && Number.isFinite(value.createdAt) &&
      age >= 0 && age < ANALYSIS_HANDOFF_TTL_MS;
    if (!valid) return null;
    const text = cleanText(value.text, 12000);
    const evidence = cleanEvidence(sourceTool, value.evidence);
    return text.trim() && evidence ? { pathId: value.pathId, sourceTool, text, evidence } : null;
  } catch {
    return null;
  }
}
