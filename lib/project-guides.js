...
import { APPLIED_PROJECT_BY_PROJECT_ID, APPLIED_PROTOTYPE_PROFILES, APPLIED_SECTORS } from "./applied-projects.js";
...
export function getPrototypeLinks(project) {
  ...
  return [
    ...
    { id: "code", label: bi("Code Builder", "أداة بناء الكود"), href: "/tools/code" },
...
export function getPrototypePromptContext(project) {
  ...
  return { projectId: project.id, idea: project.title, problem: project.problem, impact: project.impact, path: PATH_GUIDANCE[project.path].name, tools: bi(project.tools.join(", "), project.tools.join("، ")), requiredData: project.dataRequirements, expectedOutputs: project.expectedResults, evaluation: project.evaluation, applicationPotential: project.applicationPotential, solution: profile.solution, users: profile.users, inputs: profile.inputs, outputs: profile.outputs, method: profile.method, logic: profile.logic, ...(social ? { socialProblem: project.problem, affectedUsers: social.affectedUsers, languageProblem: social.languageProblem, nlpTask: bi(social.nlpTasks.join(", "), social.nlpTasks.join("، ")), annotationSchema: social.annotationSchema, expectedSocialImpact: social.expectedSocialImpact, risksAndLimitations: social.risksAndLimitations } : {}), ...(applied ? { sector: bi(APPLIED_SECTOR_NAMES[applied.sector].titleEn, APPLIED_SECTOR_NAMES[applied.sector].titleAr), practicalNeed: bi(applied.needEn, applied.needAr), appliedLanguageProblem: applied.languageProblem, appliedNlpTask: bi(applied.nlpTasks.join(", "), applied.nlpTasks.join("، ")), possibleApplication: applied.possibleApplication, expectedPracticalValue: applied.expectedPracticalValue, appliedLimitations: applied.limitations } : {}) };
}

const APPLIED_SECTOR_NAMES = Object.freeze(Object.fromEntries(APPLIED_SECTORS.map((item) => [item.id, item])));
...
export const APPLIED_PROTOTYPE_HANDOFF_FIELDS = Object.freeze([
  { id: "sector", label: bi("Possible sector", "قطاع محتمل للتطبيق") },
  { id: "practicalNeed", label: bi("Practical need", "الاحتياج التطبيقي") },
  { id: "appliedLanguageProblem", label: bi("Applied language problem", "المشكلة اللغوية التطبيقية") },
  { id: "appliedNlpTask", label: bi("Applied NLP task", "مهمة NLP التطبيقية") },
  { id: "possibleApplication", label: bi("Possible application", "التطبيق المحتمل") },
  { id: "expectedPracticalValue", label: bi("Expected practical value", "القيمة التطبيقية المتوقعة") },
  { id: "appliedLimitations", label: bi("Applied limitations", "الحدود التطبيقية") },
]);

export function getPrototypeHandoffFields(project) {
  return [...PROTOTYPE_HANDOFF_FIELDS, ...(project.socialImpact ? SOCIAL_PROTOTYPE_HANDOFF_FIELDS : []), ...(APPLIED_PROJECT_BY_PROJECT_ID[project.id] ? APPLIED_PROTOTYPE_HANDOFF_FIELDS : [])];
}
