...
export function buildPrototypeHandoff(project, language = "en", edits = {}) {
  ...
  const inherited = Object.fromEntries(PROTOTYPE_HANDOFF_FIELDS.map(({ id }) => [id, source[id]?.[language] || ""]));
  const researcherEdits = Object.fromEntries(PROTOTYPE_HANDOFF_FIELDS.map(({ id }) => [id, typeof edits[id] === "string" ? edits[id] : ""]));
  const reviewed = Object.fromEntries(PROTOTYPE_HANDOFF_FIELDS.map(({ id }) => [id, researcherEdits[id].trim() || inherited[id].trim()]));
  const missing = PROTOTYPE_HANDOFF_FIELDS.filter(({ id }) => !reviewed[id]).map(({ id }) => id);
