...
const SOURCES = new Set(["frequency", "concordance", "ngrams", "pos", "corpus-research", "interpreter", "advisor", "copilot"]);
const TYPES = new Set(["frequency", "concordance", "ngrams", "pos", "corpus-research", "interpretation", "methodology"]);
...
export function createReportContext(sourceTool, analysisType, payload, storage = globalThis?.sessionStorage, now = Date.now()) {
  ...
  const context = { version: 2, reportId, sourceTool, analysisType, pathId: payload?.pathId === "corpus-linguistics" && ["frequency", "concordance", "ngrams", "corpus-research"].includes(sourceTool) ? "corpus-linguistics" : null, timestamp: now, ...availability, payload: clean };
...
export function readReportContext(search, storage = globalThis?.sessionStorage, now = Date.now()) {
  ...
  try {
    ...
    const valid = (context?.version === 1 || context?.version === 2) && context.reportId === reportId && SOURCES.has(context.sourceTool) && TYPES.has(context.analysisType) && Number.isFinite(context.timestamp) && now >= context.timestamp && now - context.timestamp <= REPORT_CONTEXT_TTL_MS;
    if (!valid) {
