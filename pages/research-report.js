...
function contextualTitle(type, language, size) {
  const titles = {
    ...
    "reviewed-analysis": ["Reviewed research result", "تقرير النتيجة البحثية المراجعة"],
    ...
    <article className="report contextualReport" ref={reportRef}>
      ...
      {view === "diagram" ? (
        ...
        <>
          ...
          {showStandard && context.analysisType === "reviewed-analysis" && <section className="reportSection"><p className="sectionLabel">02</p><h3>{language === "ar" ? "النتيجة المراجعة" : "Reviewed result"}</h3>{payload.taskLabel && <p><strong>{language === "ar" ? "المهمة:" : "Task:"}</strong> {payload.taskLabel}</p>}{payload.sourceText && <details><summary>{language === "ar" ? "النص المصدر" : "Source text"}</summary><p dir="auto">{payload.sourceText}</p></details>}{payload.aiOutput && <><h4>{language === "ar" ? "اقتراح الذكاء الاصطناعي الأصلي" : "Original AI suggestion"}</h4><pre dir="auto">{JSON.stringify(JSON.parse(payload.aiOutput), null, 2)}</pre></>}<h4>{language === "ar" ? "قرار الباحث" : "Researcher decision"}</h4><p>{payload.researcherDecision || "—"}</p>{payload.finalOutput && <><h4>{language === "ar" ? "النتيجة النهائية المعتمدة من الباحث" : "Final researcher-validated result"}</h4><pre dir="auto">{JSON.stringify(JSON.parse(payload.finalOutput), null, 2)}</pre></>}</section>}
