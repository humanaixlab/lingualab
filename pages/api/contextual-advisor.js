...
const ALLOWED_KINDS = new Set(["path", "project", "learning"]);
...
export default async function handler(req, res) {
  ...
  const role = kind === "project" ? "Project Advisor" : kind === "learning" ? "Learning Tutor" : "Research Path Advisor";
  ...
  try {
    ...
    for await (const event of stream) {
      if (event.type === "response.output_text.delta" && event.delta) { sentText = true; writeEvent(res, "delta", { delta: event.delta }); }
      if (event.type === "response.failed") throw new Error(event.response?.error?.message || "contextual advisor stream failed");
    }
    if (!sentText) throw new Error("empty contextual advisor response");
    writeEvent(res, "done"); return res.end();
  } catch (error) {
    console.error("contextual-advisor failed", error);
    writeEvent(res, "error", { message: sentText ? "The stream ended before the answer was complete." : "The contextual advisor could not generate a reliable answer." }); return res.end();
  }
}