...
export default function PlatformInventoryPage() {
  ...
  return <main className={styles.page} dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <InventorySection title={copy.tools}><div className={styles.toolGrid}>{ACTIVE_TOOLS.map((tool) => <Link href={tool.route} key={tool.id}><strong>{tool.name[locale]}</strong><code>{tool.route}</code></Link>)}</div></InventorySection>
