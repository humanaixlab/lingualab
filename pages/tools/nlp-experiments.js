...
const COPY = {
  ar: {
    ...
    executionTitle: "كيف تعمل مهام NLP هنا؟", executionText: "ينفذ مختبر مهام NLP المهمة باستخدام نموذج ذكاء اصطناعي عبر الـAPI ضمن تعليمات وفئات وبنية مخرجات يحددها الباحث. تُعاد النتيجة بوصفها مخرجًا منظمًا (Structured Output)، ثم يخضع توافقها مع القيود إلى تحقق برمجي (Programmatic Validation)، ويظل للباحث قرار قبول النتيجة أو تعديلها أو رفضها.", executionContrast: "هذه الآلية تختلف عن الحساب الحتمي (Deterministic Computation) بخوارزمية NLP مستقلة مثل Naive Bayes أو TF-IDF أو محلل صرفي قائم على القواعد. وعندما تستخدم LinguaLab حسابًا حتميًا، يظهر ذلك بوضوح.", executionType: "نوع التنفيذ: استدلال مدعوم بالذكاء الاصطناعي (AI-supported Inference)", executionFlow: ["يحدد الباحث المهمة", "تبني الواجهة طلبًا منظمًا", "تنفذ الـAPI نموذج الذكاء الاصطناعي", "تعود استجابة JSON منظمة", "تحقق برمجي (Programmatic Validation) من النتيجة", "مراجعة الباحث (Researcher Review): قبول أو تعديل أو رفض"],
  },
  en: {
    ...
    executionTitle: "How are NLP tasks executed here?", executionText: "NLP Task Sandbox executes the task using an AI model through the API under researcher-defined instructions, labels, and output structure. The result is returned in a structured format, validated against the defined constraints, and remains subject to researcher acceptance, modification, or rejection.", executionContrast: "This differs from running an independent deterministic NLP algorithm such as Naive Bayes, TF-IDF, or a rule-based morphological analyzer. When LinguaLab uses deterministic computation, it is labeled explicitly.", executionType: "Execution type: AI-supported inference", executionFlow: ["Researcher defines task", "UI builds a structured request", "API executes an AI model", "Structured JSON response", "Programmatic validation", "Researcher Accept / Modify / Reject"],
  },
};
...
export default function NlpExperiments() {
  ...
  return <><Head><title>{copy.head} · LinguaLab</title></Head><main className={styles.page}>
    ...
    <section className={styles.executionExplanation} aria-labelledby="nlp-execution-title">
      ...
      <ol className={styles.executionFlow}>{copy.executionFlow.map((step, index) => <li key={step}><span>{index + 1}</span><strong>{step}</strong></li>)}</ol>
    </section>
    ...
    <ScientificFoundations pathId="nlp-experiments" language={locale} />
  </main></>;
}
