  ...
  return (
    <section className={styles.section} id="research-paths" aria-labelledby="research-paths-title">
      <header className={styles.header}>
        <p className={styles.eyebrow}>{heading.eyebrow}</p><h2 id="research-paths-title">{heading.title}</h2><p className={styles.subtitle}>{heading.subtitle}</p><p>{copy.positioning}</p><p className={styles.support}>{copy.support}</p>
      </header>
        ...
        {paths.map((path) => (
          <article className={styles.card} id={path.id} key={path.id}>
            ...
            <div className={`${styles.toolBlock} ${path.available.length ? styles.availableBlock : styles.unavailableBlock}`}>
              ...
              {path.available.length ? <div className={styles.tools}>{path.available.map((tool) => <Link className={tool.contextual ? styles.contextualTool : undefined} href={researchPathHref(tool.href, path.id)} key={tool.href}>{tool[locale]} <span aria-hidden="true">↗</span>{tool.contextual && <small>{copy.contextual}</small>}</Link>)}</div> : <p className={styles.unavailable}>{copy.unavailable}</p>}
            </div>
            ...
            {SCIENTIFIC_FOUNDATIONS[path.id] && <ScientificFoundations pathId={path.id} language={locale} />}
            <details className={styles.guide}><summary>{copy.beginner}</summary><p>{path.beginner[locale]}</p></details>
            <details className={styles.guide}><summary>{copy.advanced}</summary><p>{path.advanced[locale]}</p></details>
          </article>
        ))}
      </div>
    </section>
