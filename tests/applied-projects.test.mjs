...
test("applied discovery defines ten bilingual sectors and ten structured records", () => {
  ...
  const sectorIds = new Set(APPLIED_SECTORS.map((sector) => sector.id));
  for (const entry of APPLIED_PROJECT_ENTRIES) {
    ...
    const project = PROJECT_CATALOG.find((item) => item.id === entry.projectId);
    assert.ok(project, `${entry.id} has no canonical project ${entry.projectId}`);
    assert.ok(sectorIds.has(entry.sector), `${entry.id} has an unknown sector ${entry.sector}`);
    assert.ok(PROJECT_PATH_ROUTES[project.path], `${entry.id} has an unknown project path ${project.path}`);
